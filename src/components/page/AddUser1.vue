            ...
            <div class="form-box">
                <el-form ref="form" :model="form" :rules="rules" :status-icon="true" label-width="80px" @validate="validate">
                    ...
                    <el-form-item label="科目">
                        <el-select v-model="form.kemu" placeholder="请选择" filterable :multiple="true" :multiple-limit="3" :reserve-keyword="true">
                            <el-option :label="item.name" :value="item.code" v-for="(item,index) in list" :key="index"></el-option>
                        </el-select>
                    </el-form-item>
    ...
    onSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          data.callerid = bus.userinfo.uid;
          data.role = 'Teacher';
          register(data, '/api/xiti/v1/resource/addTeacher')
            .then(data => {
              this.$alert('注册成功', '', {
