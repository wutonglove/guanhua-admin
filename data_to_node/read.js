  ...
  for (var j = 0; j < _data[i].length; j++) {
    ...
    if (_cur) {
      if (j === 0 && find(data, _cur) === -1) {
        // 年级
        data.push({ code: _cur, name: njdata[_cur], data: [] });
        if (data.length > 1) {
          njindex++;
        ...
        _list.push({ code: _cur, name: xkdata[_cur], data: [] });
        if (_list.length > 1) {
          xkindex++;
        ...
        _list.push({ code: _cur, name: cdata[_cur], data: [] });
        if (_list.length > 1) {
          ceindex++;
        ...
        // jie
        var _list = data[njindex].data[xkindex].data[ceindex].data[zindex].data;
        _list.push({
          code: _data[i][7],
          name: _cur,
          data: []
        ...
        var _list =
          data[njindex].data[xkindex].data[ceindex].data[zindex].data[jindex]
            .data;
        _list.push({
          code: _data[i][7],
          name: _cur,
...
fs.writeFile('./1.json', JSON.stringify(xkdata), function(err) {
  if (err) {
    throw err;
  }
